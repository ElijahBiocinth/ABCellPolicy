...
def poly_features(img, poly: Polygon):
    ...
    if USE_INTENSITY:
        ...
        feat.extend([mean_val/255, std_val/255, p10/255, p50/255, p90/255, contrast])
    if USE_HU:
    ...
    if USE_MOMENTS:
        ...
        if cnt:
            ...
            ha = cv2.contourArea(hull) if hull is not None else area
            solidity = area/(ha+1e-6)
        else:
            solidity = 0.0
